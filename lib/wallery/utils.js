...
function createItem (filePath, options) {
	...
	return fs.readFile(filePath, 'utf-8').then(function (data) {
		...
		item.slug = slugify(item.slug || item.title || /^([^\.]+)/.exec(fileName)[1]);

		item.prototype = itemMethods;
...
function sortItemsAlpha (items) {
	return sortItems(items, 'title');
}
exports.sortItemsAlpha = sortItemsAlpha;
